...
def handle_login(message):
    ...
    if is_authenticated(user_id):
        ...
        # Send the "Start" command or open a menu here
        send_start_command_or_open_menu(user_id)
    else:
        bot.send_message(user_id, "Будь ласка, введіть ваш айді:")
        bot.register_next_step_handler(message, authenticate_user)

def send_start_command_or_open_menu(user_id):
    # Send the "Start" command or open a menu here
    # For example, sending the "Start" command:
...
def authenticate_user(message):
    ...
    if user_input in user_roles:
        ...
        bot.send_message(user_id, "Неправильний айді. Будь ласка, введіть ваш айді знову або натисніть /login:")
